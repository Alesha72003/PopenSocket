...
static PyObject* AsyncPopenSocket_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
    AsyncPopenSocket_Object* self;
    self = (AsyncPopenSocket_Object*) type->tp_alloc(type, 0);
    if (self != NULL) {
        self->handleRead = 0;
        self->timeout = 0;
        self->handleWrite = 0;
...
static int AsyncPopenSocket_init(AsyncPopenSocket_Object* self, PyObject* args, PyObject* kwds) {
    ...
    wchar_t pipename[28 + 2*8+1]; // \\.\pipe\asyncpopensocket-r-
    // wchar_t pipename2[28 + 2*8+1]; // \\.\pipe\asyncpopensocket-w-
    ...
    swprintf(pipename, 28 + 2*8 + 1, TEXT("\\\\.\\pipe\\asyncpopensocket-r-%X"), self);
    // swprintf(pipename2, 28 + 2*8 + 1, TEXT("\\\\.\\pipe\\asyncpopensocket-w-%X"), self);
...
static PyObject* AsyncPopenSocket_getclosed(AsyncPopenSocket_Object* self, void* closure) {
    if (self->closed) {
        Py_INCREF(Py_True);
        return Py_True;
    }
...
static int AsyncPopenSocket_setclosed(AsyncPopenSocket_Object* self, PyObject* value, void* closure) {
...
PyMODINIT_FUNC PyInit_AsyncPopenSocket(void) {
    PyObject* m;
    if (PyType_Ready(&AsyncPopenSocket_Type) < 0) {
        return NULL;
    }
    m = PyModule_Create(&AsyncPopenSocket_Module);
    if (m == NULL)
        return NULL;
    
    Py_INCREF(&AsyncPopenSocket_Type);
    if (PyModule_AddObject(m, "AsyncPopenSocket", (PyObject*) &AsyncPopenSocket_Type) < 0) {
        Py_DECREF(&AsyncPopenSocket_Type);
        Py_DECREF(m);
        return NULL;
    }
